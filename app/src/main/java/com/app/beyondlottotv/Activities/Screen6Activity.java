...
public class Screen6Activity extends AppCompatActivity {
    ...
    private void getSetUserData(){
        ...
        AppRepository.getUser(this, getApplicationContext(), "screen6", (status, user) -> {
            if (status) {
                AppRepository.getGamesofUser(this, user, getApplicationContext(), "screen5", (type, img) -> {
                    Log.d("Screen6Activity", "getSetUserData: ");
                    this.runOnUiThread(() -> {
                        if (type != null && img != null) {
                            setMediaData(Screen6Activity.this, type, img, "");
                        } else {
                            Toast.makeText(Screen6Activity.this, "No Data found!", Toast.LENGTH_SHORT).show();
